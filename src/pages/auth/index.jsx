...
export const Auth = () => {
 ...
 const signInWithGoogle = async () => {
   ...
   try {
     ...
     // Ignore popup cancellation (user closed popup or another popup was opened)
     if (err && err.code === "auth/cancelled-popup-request") {
       // no-op; user closed popup quickly or another popup was in progress
       // reset loading state so user can retry
       console.debug("Google sign-in cancelled by user or by concurrent popup.");
     } else if (err && err.code === "auth/popup-closed-by-user") {
       console.debug("Google sign-in popup closed by user.");
     } else {
       // Show a user-friendly alert and log the error
       // eslint-disable-next-line no-console
       console.error("Google sign-in failed:", err);
       // Use a small alert for now; could be replaced by UI feedback
   ...
   <div className="login-page">
     <div className="login-shell">
       ...
       <section className="login-card" aria-label="Login card">
         ...
         <div className="login-card__footer">Secure authentication powered by Google</div>
       </section>
     </div>
   </div>
 );
